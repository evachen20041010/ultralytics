...
# 將車位多邊形和已佔用多邊形可視化
def visualize_parking_spots(frame, json_data, occupied_polygons):
    for spot in json_data:
        # 將車位座標轉為 numpy array，方便繪製多邊形
        spot_polygon_points = np.array(spot["points"], dtype=np.int32)
        spot_polygon = Polygon(spot["points"])

        # 根據車位是否被佔用決定顏色
        if any(spot_polygon.intersects(occupied_polygon) for occupied_polygon in occupied_polygons):
            # 已佔用，畫粉色
            color = (255, 0, 255)  # 粉色 (BGR)
        else:
            # 未佔用，畫藍色
            color = (255, 0, 0)  # 藍色 (BGR)

        # 繪製多邊形
        cv2.polylines(frame, [spot_polygon_points], isClosed=True, color=color, thickness=2)

        # 在多邊形中央標記ID
        text_position = np.mean(spot_polygon_points, axis=0).astype(int)
        cv2.putText(frame, spot["id"], tuple(text_position), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255), 2)

    # 繪製佔用的多邊形 (可選，與上方功能重疊)
    for occupied_polygon in occupied_polygons:
        occupied_polygon_points = np.array(list(occupied_polygon.exterior.coords), dtype=np.int32)
        cv2.polylines(frame, [occupied_polygon_points], isClosed=True, color=(0, 0, 255), thickness=2)
